...
## Descriptive Table
## Summarizes demographic and analytic characteristics by sex (NHANES 2003–2006).
...
# Variables for descriptive summary
vars <- c("RIDAGEYR", "BMXBMI", "sodium", "potassium", "BPXSAR")

# Create TableOne object (omit tests)
tab1 <- CreateTableOne(vars = vars, strata = "RIAGENDR", data = nhanes,
                       addOverall = TRUE, test = FALSE)

# Numeric summary for range/quantiles
tab_summary <- nhanes |>
  select(RIAGENDR, all_of(vars)) |>
  group_by(RIAGENDR) |>
  summarise(across(
    everything(),
    list(
...
tab_long <- tab_summary |>
  pivot_longer(-RIAGENDR, names_to = c("Variable", ".value"), names_sep = "_") |>
  mutate(across(where(is.numeric), \(x) round(x, 2))) |>
  mutate(
    Range = paste0(min, " – ", max),
    Quantiles = paste0(q1, " / ", median, " / ", q3)
  ) |>
  select(
    Sex = RIAGENDR,
    Variable,
...
# Output formatted descriptive table
kable(
  tab_long,
  caption = "Descriptive Table 1. Characteristics of the analytic sample (NHANES 2003–2006) by sex.",
  booktabs = TRUE,
  align = "lccccc"
) |>
...
## Exploratory visualization: Age–BMI relationship by sex
...
nhanes <- nhanes |>
  mutate(age_group = cut(RIDAGEYR,
                         breaks = quantile(RIDAGEYR, probs = seq(0, 1, 0.1), na.rm = TRUE),
                         include.lowest = TRUE,
                         labels = paste0("Q", 1:10)))

# Boxplot: BMI vs age decile by sex
ggplot(nhanes, aes(x = age_group, y = BMXBMI, fill = factor(RIAGENDR))) +
  geom_boxplot(outlier.size = 0.8, alpha = 0.7) +
  scale_fill_manual(values = c("#5DADE2", "#F5B7B1"),
                    name = "Sex", labels = c("Male", "Female")) +
  labs(
    title = "Distribution of BMI across Age Deciles by Sex",
...
The figure shows a modest positive association between age and BMI up to midlife, with males exhibiting slightly higher median BMI than females across most age deciles.

At older ages, both sexes display greater variability and a mild plateauing of median BMI, suggesting potential nonlinearity and heteroscedasticity in the age–BMI relationship that will later be modeled using natural splines.
...
## 7.1 Main model sequence (Table 2a)
...
knitr::kable(
  table1a,
  caption = "Table 2a. Associations between dietary sodium and potassium and blood pressure in NHANES 2003–2006. The units for all coefficients are mmHg/g/day. Standard errors are given in parentheses.",
  booktabs = TRUE,
  align = c("l","c","c","c","c")
)
...
The estimates in Table 2a reproduce the sequence of models discussed in Lumley’s *Complex Surveys* chapter on NHANES 2003–2006. To make the correspondence explicit, we include below the original table from the textbook.

![](images/clipboard-3948312674.png)

Comparing our Table 2a with the textbook version, several points are worth noting:

1.  **Model structure is the same.** Both tables report an unadjusted model, then models adjusted for age, age + gender, and age + gender + BMI.
2.  **Signs of the key coefficients match.** In both versions, sodium is negative in the unadjusted model but becomes positive after adjusting for age, which is exactly the confounding story the textbook emphasizes.
3.  **Potassium remains protective in systolic BP models.** Our potassium estimates are slightly larger in magnitude but point in the same direction.
4.  **Small numerical differences are expected.** We are using data pulled through the Epiconnector/NHANES database and we applied explicit missing-value filtering before constructing the survey design. This can change the effective sample size and therefore the standard errors.
5.  **Interpretation is unchanged.** After adjusting for age (and further for sex and BMI), both analyses conclude that higher sodium intake is associated with *higher* systolic blood pressure, not lower.
...
plot(fitted(m3), resid(m3),
     main = "Model 3: Fully Adjusted (SBP)",
     xlab = "Fitted values",
     ylab = "Residuals",
     pch = 19, cex = 0.5)
...
-   Adjusting for age reverses the unadjusted coefficients for both nutrients, confirming that **age is a confounder** in this relationship.

-   Further adjustment for sex and BMI produces little change, indicating that age is the primary source of confounding.
...
## 7.2. Sensitivity: Natural spline for age (Table 2b)
...
plot(fitted(m1), resid(m1), main = "Linear age (Model 1 on SBP)",
xlab = "Fitted", ylab = "Residuals")
lines(lowess(fitted(m1), resid(m1)), col = "blue", lwd = 2)

plot(fitted(m1_ns), resid(m1_ns), main = "Spline age (Model 1_ns on SBP)",
xlab = "Fitted", ylab = "Residuals")
lines(lowess(fitted(m1_ns), resid(m1_ns)), col = "blue", lwd = 2)

plot(fitted(m2_ns_int), resid(m2_ns_int), main = "Spline + Age*Sex (Model 2_ns_int on SBP)",
xlab = "Fitted", ylab = "Residuals")
...
### 7.2.3 Coefficient summary: Table 2b

Following the residual diagnostics above, we summarize the estimated associations between sodium, potassium, and blood pressure using the spline-based age specification. Table 2b reports coefficients for both SBP and DBP models. The directions and magnitudes of the nutrient effects remain consistent with the linear-age results, while model fit improves slightly due to the nonlinear treatment of age. This indicates that introducing a spline for age refines the model’s flexibility without materially altering the core associations observed earlier.
...
# ---------- Table 2b: Natural spline for age (no interaction) ----------
...
k1b <- knitr::kable(
  table1b,
  caption = "Table 2b. Sodium and potassium associations using a natural spline for age (mmHg/g/day; SE in parentheses).",
  booktabs = TRUE,
  align = c("l","c","c","c","c")
)
...
-   Using a natural spline for age yields **very similar coefficients** to the linear-age models in Table 2a.
...
## 7.3. Sensitivity: Natural spline age × sex (Table 2c)
...
plot(fitted(m3_ns_int), resid(m3_ns_int),
     main = "Spline × Sex Interaction on SBP",
     xlab = "Fitted values", ylab = "Residuals")
...
```{r}
# ---------- Table 2c: Spline age with Age × Sex interaction ----------
...
k1c <- knitr::kable(
  table1c,
  caption = "Table 2c. Sodium and potassium associations with natural spline age and an age×sex interaction (mmHg/g/day; SE in parentheses).",
  booktabs = TRUE,
  align = c("l","c","c","c","c")
)
if (requireNamespace("kableExtra", quietly = TRUE)) {
...
The residual comparison indicates that introducing the spline × sex interaction reduces curvature and heteroscedasticity, particularly among older participants. Table 2c summarizes the corresponding regression estimates. The directions and magnitudes of the sodium and potassium coefficients remain consistent with previous spline-only models, suggesting that the improvement in model fit stems mainly from capturing sex-specific aging patterns rather than changes in the nutrient effects.
...
-   The direction and significance of the effects remain consistent with the spline-only models (Table 2b).
...
-   Thus, the large sign reversal seen between unadjusted and adjusted models (Table 2a) stems from **age confounding itself**, not from sex-specific nonlinearity in age.
...
-   **Linear age models** (Table 2a)

-   **Spline age models** (Table 2b)

-   **Spline × Sex models** (Table 2c)
...
```{r}
# Combine Table 2a, 2b, and 2c for comparison
table_all <- dplyr::bind_rows(
  dplyr::mutate(table1a, Model_Type = "Linear age (Table 2a)"),
  dplyr::mutate(table1b, Model_Type = "Spline age (Table 2b)"),
  dplyr::mutate(table1c, Model_Type = "Spline age × Sex (Table 2c)")
)
...
k_all <- knitr::kable(
  table_all,
  caption = "Tbale 2d. Comparison of linear, spline, and spline×sex models for sodium and potassium associations with blood pressure (mmHg/g/day; SE in parentheses).",
  booktabs = TRUE,
  align = c("l","l","c","c","c","c")
)
...
-   **Interpretation summary:**
    ...
    -   The spline models (Table 2b) yield **slightly smaller standard errors**, indicating a small improvement in model flexibility but no major shift in effect size.

    -   Including the **age × sex interaction** (Table 2c) leaves the main findings virtually unchanged, suggesting that sex-specific aging patterns have little influence on these nutrient–blood pressure relationships.
...
```{r 8_1_subset, echo=TRUE, message=FALSE, warning=FALSE}
...
A near-flat smoother suggests no major model misspecification. If curvature were visible, we might consider adding an age × sex interaction or using spline terms, as explored in Section 7.
...
```{r}
# Error in termplot() "Error in plot.window(...) : invalid 'xlim' value", so we opt to the following:
...
plot(fitted(m3), resid(m3),
main = "No Interaction (Model 3 on SBP)",
xlab = "Fitted values", ylab = "Residuals")
lines(lowess(fitted(m3), resid(m3)), col = "blue", lwd = 2)

# Model with age × sex interaction
...
**Interpretation:** Adding the age × sex interaction yields a smoother and flatter residual trend, particularly among older participants, confirming improved fit. This supports the earlier conclusion that interaction and spline adjustments modestly enhance model adequacy.
...
-   **Including the age × sex interaction** further stabilizes residual trends.
...
## 9.3. Table 3. Age interactions summary
...
The estimates in Table 3 closely mirror those in Lumley’s *Complex Surveys* (Table 5.2), with nearly identical coefficients and standard errors. Minor differences reflect updated weighting and stricter missing-value handling, but the interpretation is unchanged: both sodium and potassium show modest, age-dependent attenuation of their associations with blood pressure.

![](images/clipboard-1497767788.png)
